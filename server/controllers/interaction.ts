...
const apiKey = "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";
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, __dirname + "/../uploads"); // 음성 파일을 저장할 폴더 경로 지정
    },
...
async function convertTexttoSpeech(
    ...
    try {
        ...
        await writeFile(
            `npc_audio/${audioFileName}`,
            response_audio.audioContent,
            "binary"
        );
        const audioFileUrl: string = `${serverUrl}npc_audio/${audioFileName}`;
